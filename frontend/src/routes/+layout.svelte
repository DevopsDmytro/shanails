<script lang="ts">
	...
	$: if (!initializing && $page.url.pathname !== "/register") {
		if (!$authStore.isAuthenticated) {
			...
			if (isTelegram()) {
				...
				// Regular web browser (or Telegram browser without initData) - redirect to Telegram bot
...
<main>
	...
	{:else}
		{#if $page.url.pathname !== "/booking" && $page.url.pathname !== "/register"}
			<h1>Салон Красоти</h1>
			<nav>
